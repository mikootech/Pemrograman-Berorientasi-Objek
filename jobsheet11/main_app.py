...
def get_anggaran_manager():
    print(">>> STREAMLIT: Menginisialisasi AnggaranHarian... (Cache Resource)")
    ...
    with st.form("form_transaksi_baru", clear_on_submit=True):
        ...
        if submitted:
            if not deskripsi: 
                ...
                st.warning("Jumlah wajib dan positif!", icon="⚠️")
            else:
...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    # Ambil list object untuk mendapatkan ID asli di database
    transaksi_obj = anggaran.get_semua_transaksi_obj()
    
    if not transaksi_obj:
        st.info("Belum ada transaksi.")
        return

    # Membuat Dataframe custom untuk menampilkan ID agar user tahu mana yang mau dihapus
    data = []
    for tx in transaksi_obj:
        data.append({
            "ID Transaksi": tx.id,
            "Tanggal": tx.tanggal.strftime("%Y-%m-%d"),
            "Kategori": tx.kategori,
            "Deskripsi": tx.deskripsi,
            "Jumlah": format_rp(tx.jumlah)
        })
    df_lengkap = pd.DataFrame(data)
    
    st.dataframe(df_lengkap, use_container_width=True, hide_index=True)
    ...
    with col_hapus1:
        id_yang_dihapus = st.number_input("ID Transaksi Hapus:", min_value=1, step=1)
        
    with col_hapus2:
        st.write("") # Spacer agar tombol turun sedikit sejajar input
        st.write("")
        if st.button("Hapus Transaksi Terpilih"):
            st.warning(f"Apakah Anda yakin ingin menghapus transaksi dengan ID {id_yang_dihapus}?", icon="⚠️")
            
            # Sub-button konfirmasi
            if st.button("Konfirmasi Hapus"):
                if anggaran.hapus_transaksi(id_yang_dihapus):
                    st.success(f"Transaksi ID {id_yang_dihapus} berhasil dihapus.")
